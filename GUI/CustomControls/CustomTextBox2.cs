...
namespace GUI.CustomControls
{
    ...
    public partial class CustomTextBox2 : UserControl
    {
        ...
        public void setRule()
        {
            // Thay thế PlaceHolder bằng Text của textBox1 nếu giống nhau
            if (Texts.Equals(textBox1.Text))
            {
                Texts = string.Empty;
            ...
            // Sao chép logic từ phương thức setRule trong CustomTextbox của bạn
            if (Texts.Equals(textBox1.Text))
                return;

            if (Texts.Length > length)
            {
                int c = Texts[Texts.Length - 1];
                if (!(c >= 48 && c <= 57 || c >= 65 && c <= 90 || c >= 97 && c <= 122))
                    Texts = Texts.Substring(0, Texts.Length - 1);
            }
            length = Texts.Length;
            if (Texts.Length > 0)
                textBox1.SelectionStart = Texts.Length;
        }
